...
function runRuleEngine() {
    try {
        ...
        // 防護：焓值為 0、NaN 或 undefined 時顯示警告
        if (!h && h !== 0 || isNaN(h)) {
            document.getElementById("rule-output").innerHTML = `
            <div class="rule-box" style="border-left-color: #bdc3c7;">
                <span class="rule-title" style="color:#bdc3c7">⚠️ 無法取得焓值資料</span>
                <div class="rule-content">請檢查天氣資料來源是否正常。</div>
            </div>`;
            return;
        }
...
document.addEventListener('DOMContentLoaded', function () {
    // 頁面載入時立即用預設值執行一次規則引擎，避免 API 失敗時 SOP 空白
    runRuleEngine();
    ...
    // ==========================================
    // PWA 鍵盤修復 (iOS Safari WebView)
    // ==========================================
    const isIOSPWA = window.navigator.standalone === true;
    const isAndroidPWA = window.matchMedia('(display-mode: standalone)').matches;

    if (isIOSPWA || isAndroidPWA) {
        console.log('📱 PWA 模式偵測，啟用鍵盤修復');
