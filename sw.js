...
const CACHE_NAME = 'chiller-ai-v13';  // SW 也用 CWA 優先 + proxy 驗證
const urlsToCache = [
  './',
  './index.html',
  './manifest.json',
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (text.startsWith('error') || text.includes('error code')) {
          console.warn(`SW: Proxy 回傳錯誤 (${proxy.split('/')[2]}): ${text.substring(0, 60)}`);
          continue;
        }
...
// CWA API Key（與主頁同步）
const SW_CWA_API_KEY = 'CWA-1140C840-6DB4-43F4-B08C-8782C7305D72';
const SW_CWA_STATION_ID = '466920';

// 取得天氣預報並檢查預警（優先 CWA）
async function fetchForecastAndCheck() {
  // 優先嘗試 CWA（直連成功率高）
  try {
    const cwaUrl = `https://opendata.cwa.gov.tw/api/v1/rest/datastore/O-A0003-001?Authorization=${SW_CWA_API_KEY}&StationId=${SW_CWA_STATION_ID}`;
    const res = await swFetchWithProxy(cwaUrl);
    if (res.ok) {
      ...
      if (data.success && data.records?.Station?.[0]) {
        ...
        if (!isNaN(temp) && !isNaN(hum) && temp !== -99 && hum !== -99) {
          ...
          console.log(`SW: CWA 天氣成功 ${temp}°C, ${hum}%`);
          return { temp, hum, enthalpy, rule: getRule(enthalpy) };
        }
      }
    }
  } catch (e) {
    console.warn('SW: CWA 取得失敗', e.message);
  }
  ...
  try {
    ...
    if (temp !== undefined && hum !== undefined) {
      const enthalpy = calculateEnthalpy(temp, hum);
      console.log(`SW: Open-Meteo 天氣成功 ${temp}°C, ${hum}%`);
      return { temp, hum, enthalpy, rule: getRule(enthalpy) };
    }
  } catch (e) {
