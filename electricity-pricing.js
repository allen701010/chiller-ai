 ...
 * 取得當前時段類型與電價
 * @param {Date} date 
 * @returns {{type: string, typeName: string, price: number, season: string}}
 */
function getCurrentPricing(date = new Date()) {
    ...
    if (pricing.peak.price && isInPeriod(hour, pricing.peak.periods)) {
        return { type: 'peak', typeName: '尖峰', price: pricing.peak.price, season: seasonName };
    }
    if (pricing.semiPeak.price && isInPeriod(hour, pricing.semiPeak.periods)) {
        return { type: 'semiPeak', typeName: '半尖峰', price: pricing.semiPeak.price, season: seasonName };
    }
    if (pricing.offPeak.price && isInPeriod(hour, pricing.offPeak.periods)) {
        return { type: 'offPeak', typeName: '離峰', price: pricing.offPeak.price, season: seasonName };
    }

    // 預設回傳離峰
    return { type: 'offPeak', typeName: '離峰', price: pricing.offPeak.price || 2.32, season: seasonName };
}
...
function updatePricingDisplay() {
    ...
    container.innerHTML = `
        <div class="pricing-current" style="border-left: 4px solid ${color};">
            <div class="pricing-type" style="color: ${color};">${pricing.typeName}</div>
            <div class="pricing-value">${pricing.price.toFixed(2)} <small>元/度</small></div>
            <div class="pricing-season">${pricing.season}</div>
        </div>
    `;
    ...
    if (seasonTag) {
        seasonTag.textContent = pricing.season;
    }
}
