...
// 當頁面變為可見時，檢查是否錯過了任何通知
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
        const now = Date.now();
        const timeSinceLastCheck = now - lastVisibilityCheckTime;

        console.log(`👁️ 頁面切回前台，距離上次檢查: ${Math.round(timeSinceLastCheck / 1000)} 秒`);

        // 如果超過 1 分鐘沒有檢查，立即執行檢查
        if (timeSinceLastCheck > 60000) {
            console.log('⚠️ 偵測到可能錯過的通知，立即檢查...');
            checkReminder();

            // 更新天氣和預測資料
            if (typeof getNangangWeather === 'function') {
                getNangangWeather();
            }
...
window.addEventListener('focus', () => {
    ...
    if (timeSinceLastCheck > 60000) {
        console.log('👁️ 頁面獲得焦點，距離上次檢查超過1分鐘，立即檢查...');
        checkReminder();
    }
