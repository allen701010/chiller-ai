...
async function tryCWA_API() {
    ...
    try {
        const res = await fetch(
            `https://opendata.cwa.gov.tw/api/v1/rest/datastore/O-A0003-001?Authorization=${CWA_API_KEY}&StationId=${CWA_STATION_ID}`,
...
async function tryWttrAPI() {
    ...
    try {
        const res = await fetch('https://wttr.in/25.04294,121.61377?format=j1', {
            signal: controller.signal
        });
        clearTimeout(timeoutId);
